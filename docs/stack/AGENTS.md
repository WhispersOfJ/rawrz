...
**Repository containment rule (RAWRZ):** Every worktree for this repository must
...
## Linkage note (RAWRZ — updated)

The RAWRZ stack is the media backbone. A web-based RPG whose mechanics revolve
around watching movies/TV from this stack is **part of RAWRZ** (see
`rawrz-megastack-spec.md` §10): it will be a Compose service in the merged stack,
not a separate project. Its spec lives at [docs/rpg/movie-rpg-spec.md](docs/rpg/movie-rpg-spec.md).
When working on the RPG, bring that spec into context; when working on the stack,
do not assume the RPG is separate.

> **⚠ Superseded:** the old "linked but not part of" language (no new container,
> separate lifecycle) was reversed by RAWRZ master spec §10. See the supersession
> banner in `docs/rpg/movie-rpg-spec.md`.
