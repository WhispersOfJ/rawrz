...
- **Migration placement (2026-09-08):** `genre_xp_ledger` is created in the `watches` migration (0006), not with the other genre tables (0004) — its `source_watch_id` foreign key targets `watches(id)`, the same deferred-prerequisite pattern used for `sync_state` (0002 → 0003).
